...
function generateRandomName() {
    let names = ["Dominique", "David", "Mamadou", "Adda", "Alfred", "Ted"];
    let rand = Math.floor(Math.random() * names.length);
    return names[rand];
}

function generateRandomFirstName() {
    let firstNames = ["JETLI", "KRILIN", "GOLIATH", "ABDOUL", "KRADOUK"];
...
function generateRandomCity() {
    let cities = ["Paris", "Toulouse", "Nancy", "Metz"];
...
function generation(numberOfEmployees) {
    MongoClient.connect(url, function(err, db) {
        ...
        dbo.collection("employee").insertMany(employees, function(err, res) {
          if (err) throw err;
